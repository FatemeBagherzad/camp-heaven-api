...
const getOne = (table) =>
  catchAsync(async (req, res, next) => {
    ...
    if (table === 'users') {
      if (doc.photo) {
        doc.photo = `${process.env.BACKEND_URL}/img/users/${doc.photo}`;
      }
    }
...
//         obj.imageCover = `${process.env.BACKEND_URL}/img/camps/${a}`;
//         obj.images[0] = `${process.env.BACKEND_URL}/img/camps/${b}`;
//         obj.images[1] = `${process.env.BACKEND_URL}/img/camps/${c}`;
//         obj.images[2] = `${process.env.BACKEND_URL}/img/camps/${d}`;
//       }
//     });
...
const getAll = (table) =>
  catchAsync(async (req, res, next) => {
    ...
    if (table === 'camps') {
      doc.forEach((obj) => {
        if (obj.imageCover) {
          obj.imageCover = `${process.env.BACKEND_URL}/img/camps/${obj.imageCover}`;
        }
        ...
        if (obj.images) {
          ...
          obj.images = images.map(
            (image) =>
              `${process.env.BACKEND_URL}/img/camps/${image}`
          );
        }
      });
    }
